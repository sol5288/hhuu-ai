...
onMounted(() => {
	...
	baseImage.onload = () => {
		...
		computedHeight.value =
			props.height || (ratio < 1 ? baseImage.height * ratio : baseImage.height);
			...
			paths.value.forEach((pathArr: any[]) => {
				pathArr.forEach((path, index) => {
					if (path.type === 'begin' || path.type === 'draw') {
						if (index === 0 || pathArr[index - 1].type !== path.type) exportCtx.beginPath();
				...
				if (pathArr[pathArr.length - 1].type !== 'begin') exportCtx.stroke();
			});
...
function redrawCanvas() {
	...
	paths.value.forEach((pathArr: any[]) => {
		pathArr.forEach((path, index) => {
			if (index === 0 || pathArr[index - 1].type !== path.type) ctx.beginPath();
			...
			ctx.stroke();
			if (index === pathArr.length - 1 || pathArr[index + 1].type !== path.type) ctx.closePath();
		});
	});
	ctx.globalCompositeOperation = 'source-over';
}
