...
async function queryChatBox() {
	...
	if (res?.data && res?.data.length) boxData.value = res?.data;
	else boxData.value = defaultChatBox;
}
...
function handleClick(box) {
	...
	if (appId && appId > 0) router.push({ path: '/chat', query: { appId } });
	else emit('prompt', prompt);
}
	...
	<div :class="[isMobile ? 'mt-2' : 'mt-28']" class="rounded-md px-4 py-4">
		...
		<div class="w-full md:min-w-[450px]">
			<div
				...
				:style="{ gridTemplateColumns: `repeat(${boxData.length}, minmax(0, 1fr))` }"
			>
				<div v-for="item in boxData" :key="item.id" class="space-y-4">
					...
					<div v-for="box in item.childList" :key="box.id" class="space-y-e">
						<div
							class="py-3 flex justify-center items-center rounded cursor-pointer select-none bg-neutral-100 hover:bg-neutral-200 dark:bg-neutral-900 dark:hover:bg-neutral-800"
