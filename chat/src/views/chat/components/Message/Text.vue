...
const headerStyle = computed(() => (appStore.theme === 'dark' ? 'header-2' : 'header-1'));

const codeStyle = computed(() => (appStore.theme === 'dark' ? 'code-2' : 'code-1'));
...
function highlightBlock(str: string, lang?: string) {
	return `<pre class="code-block-wrapper ${codeStyle.value}"><div class="code-block-header ${headerStyle.value}">
          ...
          /> -->
				...
				<div v-else>
					...
					<NImage
						...
						v-if="imageUrl && isImageUrl"
					/>
					<a :href="imageUrl" target="_blank" :class="{ 'file-2': isMobile, 'file-1': !isMobile }">
						<img
