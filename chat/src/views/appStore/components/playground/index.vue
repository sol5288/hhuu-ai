...
const conversationOptions = computed(() =>
	answerList.value
		.filter((item) => item.conversationOptions)
		.map((t) => (t.conversationOptions ? JSON.parse(t.conversationOptions) : {}))
);
...
async function queryAnswerByAppId() {
	...
	answerList.value = res.data.rows
		.map((t) => {
			t.loading = false;
			return t;
		})
...
async function handleRun() {
	...
	try {
			...
			await fetchChatAPIProcess<Chat.ConversationResponse>({
				...
				onDownloadProgress: ({ event }) => {
					...
					try {
						...
						if (userBanance) authStore.updateUserBanance(userBanance);
		...
		if (error?.message) updateMockAnswer(curId, { answer: error?.message });
	}
...
<template>
	<div class="w-full h-full ground flex justify-between" :class="[isMobile ? 'flex-col' : '']">
		<div
			class="ground-left flex flex-col h-full"
			:class="[isMobile ? 'w-full' : 'w-6/12 border-[#0000000a] border-r dark:border-[#ffffff17]']"
		>
			<div class="ground-left-input select-none">
				...
				<div
					class="flex py-4 px-4 bg-[#10b9810a]"
				...
				>
					...
					<div
						class="run-btn flex flex-col justify-center items-center rounded-md px-16 py-1 select-none"
			...
			<div class="flex-1 mt-4flex flex-col mt-6">
				...
				<div class="flex-1 overflow-y-scroll pl-2 pr-5 py-4" :class="[isMobile ? '' : 'h-[150px]']">
					<div
						...
						:key="index"
						class="border dark:border-[#ffffff17] px-3 py-1 rounded-md mb-2 flex justify-between items-center cursor-pointer transition hover:border-[#5a91fc] hover:text-[#5a91fc]"
						@click="useDemo(item)"
					>
		...
		<div class="flex flex-col" :class="[isMobile ? 'w-full' : 'w-6/12']">
			<div class="ground-right-header p-2 flex justify-between items-center">
				<div class="flex items-center px-3 pt-2">
					<span
						class="w-10 h-10 flex justify-center items-center rounded-md shadow-md mr-5 border border-[#00000014]"
					>
				...
				<span
					class="w-6 h-6 hover:bg-neutral-100 cursor-pointer flex justify-center items-center"
					@click="handleClose"
				>
			...
			<div
				...
				id="scrollRef"
				ref="scrollRef"
				class="flex-1 px-5 py-4 overflow-y-scroll"
			>
				<div ref="playgroundTopRef" />
				<div
					v-for="(item, index) in answerList"
					:key="item.id"
					class="mb-8 border rounded-md dark:border-[#ffffff17]"
					:class="[item.loading ? 'border-[#5a91fc]' : '']"
				>
					<div
						class="flex p-4 bg-[#f6fcfa] rounded-md dark:bg-[#18181c] max-h-[120px] overflow-hidden"
					>
					...
					<div
						class="flex p-4 border-b-2 border-dotted border-gray-200 dark:border-[#ffffff17] min-h-[80px]"
					>
