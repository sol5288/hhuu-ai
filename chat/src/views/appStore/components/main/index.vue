...
watch(catId, (val) => {
	...
	else activeList.value = appList.value.filter((item: App) => item.catId === val);
});
...
<template>
	<div class="relative flex justify-center">
		<div class="w-full flex flex-col items-center max-w-screen-4xl p-4 lg:p-6">
			...
			<div
				class="sticky left-0 mt-3 right-0 top-0 flex max-w-screen-4xl items-center z-50 w-full bg-[#f3f4fc] dark:bg-[#101014]"
			>
					...
					>
						...
						<div class="w-full flex justify-between mt-1">
							...
							>
								<template #icon>
									<SvgIcon
										:icon="isMineApp(item) ? 'iconamoon:sign-minus-bold' : 'mi:add'"
										class="text-base"
									/>
								</template>
								{{
