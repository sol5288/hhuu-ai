...
export function fetchGetQRSceneStrAPI<T>(data: { invitedBy?: string }): Promise<T> {
	return post<T>({
...
/* login by scenceStr */
export function fetchLoginBySceneStrAPI<T>(data: { sceneStr: string }): Promise<T> {
	return post<T>({
...
export function fetchBindWxBySceneStrAPI<T>(data: { sceneStr: string }): Promise<T> {
	return post<T>({
...
export function fetchInviteCodeAPI<T>(data: { code: string }): Promise<T> {
	return get<T>({
		url: '/user/inviteLink',
		data,
	});
