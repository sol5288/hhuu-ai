...
function isDateDisabled(timestamp: number) {
	if (!isThisMonth(timestamp)) return true;
	return false;
}
...
function signed(month: number, date: number) {
	if (!signInData.value.length) return false;
...
async function handleSignIn() {
	try {
		...
		if (res.success) ms.success('签到成功！');
		getSigninLog();
...
<template>
	<NModal
		:show="visible"
		:style="{ maxWidth: '650px', minWidth: isSmallLg ? '100%' : '650px' }"
		...
		<NSpace vertical>
			<NCard closable @close="handleClose">
				...
				<NAlert class="mb-5 p-0 !bg-[#ccddff]" :show-icon="false" type="primary">
					每日签到赠送：
					<span v-if="signInModel3Count > 0"
						><b class="text-[red]">{{ signInModel3Count }}</b
						>积分基础模型对话额度</span
					>
					<span v-if="signInModel4Count > 0"
						><b class="ml-2 text-[red]">{{ signInModel4Count }}</b
						>积分高级模型对话额度</span
					>
					<span v-if="signInMjDrawToken > 0"
						><b class="ml-2 text-[red]">{{ signInMjDrawToken }}</b
						>点绘画积分额度</span
					>
				</NAlert>
				<NSpin :show="loading">
