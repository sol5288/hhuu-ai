...
const rules = reactive<FormRules>({
  ...
  des: [{ required: true, message: '请填写分类描述', trigger: 'blur' }],
  ...
  <div>
    ...
    <page-main>
      <el-form ref="formRef" :inline="true" :model="formInline">
        <el-form-item label="分类名称" prop="name">
          <el-input v-model="formInline.name" placeholder="分类名称[模糊搜索]" @keydown.enter.prevent="queryCatList" />
        </el-form-item>
    ...
    <page-main style="width: 100%;">
      <el-table v-loading="loading" border :data="tableData" style="width: 100%;" size="large">
        <el-table-column prop="coverImg" label="套餐封面" width="120">
          <template #default="scope">
            <el-image style="height: 50px;" :src="scope.row.coverImg" fit="fill" />
          </template>
        </el-table-column>
        <el-table-column prop="name" label="分类名称" />
        <el-table-column prop="appCount" label="app数量" />
        ...
        <el-table-column label="操作" width="200">
          <template #default="scope">
            ...
            <el-popconfirm title="确认删除此分类么?" width="200" icon-color="red" @confirm="handleDeletePackage(scope.row)">
              <template #reference>
    ...
    <el-dialog v-model="visible" :close-on-click-modal="false" :title="dialogTitle" width="570"
      ...
      <el-form ref="formPackageRef" label-position="right" label-width="100px" :model="formPackage" :rules="rules">
        ...
        <el-form-item label="套餐描述" prop="des">
          <el-input v-model="formPackage.des" type="textarea" placeholder="请填写分类介绍信息、用于对外展示..." :rows="4" />
        </el-form-item>
      </el-form>
      <template #footer>
