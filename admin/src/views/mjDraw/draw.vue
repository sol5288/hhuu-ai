      ...
      <el-form ref="formRef" :inline="true" :model="formInline">
        ...
        <el-form-item label="绘制状态" prop="status">
          <el-select v-model="formInline.status" placeholder="请选择绘制状态" clearable>
            <el-option v-for="item in WITHDRAW_STATUS_OPTIONS" :key="item.value" :label="item.label"
              :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <page-main style="width: 100%">
      <el-table v-loading="loading" border :data="tableData" style="width: 100%" size="large">
        ...
        <el-table-column prop="drawUrl" align="center" label="绘图结果">
          <template #default="scope">
            <el-image style="height: 120px" preview-teleported fit="contain" :preview-src-list="[scope.row?.drawUrl]"
              :src="scope.row?.drawUrl" lazy hide-on-click-modal />
          </template>
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="200" align="center">
          <template #default="scope">
            <el-popconfirm :title="`确认${scope.row.rec === 1 ? '取消推荐' : '推荐'
              }图片吗`" width="260" icon-color="red" @confirm="recommendDrawImg(scope.row.id)">
              <template #reference>
      ...
      <el-row class="flex justify-end mt-5">
        <el-pagination v-model:current-page="formInline.page" v-model:page-size="formInline.size" class="mr-5"
          ...
          @size-change="queryDrawList" @current-change="queryDrawList" />
      </el-row>
    </page-main>
  </div>
</template>
