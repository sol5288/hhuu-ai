  ...
  <div>
    <page-main>
      <el-alert :closable="false" show-icon title="动态菜单配置说明"
        description="请至少配置一个、如果不配置默认设置对话页面、排序ID越小越靠前、PC和移动端是两套独立的设置、图标选择请在此处： https://icon-sets.iconify.design/ 找到或搜索到喜欢的图标之后可以复制下方的图标编号 例如material-symbols:chat-outline、更加详细文档参考官方演示站公告文档地址"
        type="success" />
    </page-main>
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <div class="flex items-center">
              ...
              <el-button @click="handleShowMenu(scope.row)" v-if="!scope.row.menuType"
                :type="scope.row.isShow ? 'danger' : 'success'" size="small">{{ scope.row.isShow ? '隐藏菜单' : '打开菜单'
                }}</el-button>
    ...
    <el-dialog v-model="visible" :close-on-click-modal="false" :title="dialogTitle" width="570"
      ...
      <el-form ref="formRef" label-width="140" :rules="rules" :model="formInline">
        ...
        <el-form-item label="是否打开新窗口" prop="isJump">
          <el-switch v-model="formInline.isJump" />
        </el-form-item>
        <el-form-item label="是否需要登录访问" prop="isNeedAuth">
          <el-switch v-model="formInline.isNeedAuth" />
        </el-form-item>
      </el-form>
      <template #footer>
