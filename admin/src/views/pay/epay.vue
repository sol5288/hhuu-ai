...
const rules = ref<FormRules>({
  ...
  payEpaySecret: [{ required: true, trigger: 'blur', message: '请填写支付秘钥' }],
  payEpayPid: [{ required: true, trigger: 'blur', message: '请填写商户PID' }],
  payEpayNotifyUrl: [{ required: true, trigger: 'blur', message: '请填写支付通知地址' }],
  payEpayApiPayUrl: [{ required: true, trigger: 'blur', message: '请填写平台支付API请求地址' }],
  payEpayApiQueryUrl: [{ required: true, trigger: 'blur', message: '请填写平台API商户查询地址' }],
})
...
async function queryAllconfig() {
  const res = await apiConfig.queryConfig({ keys: ['payEpaySecret', 'payEpayNotifyUrl', 'payEpayReturnUrl', 'payEpayPid', 'payEpayStatus', 'payEpayApiPayUrl', 'payEpayApiQueryUrl', 'payEpayRedirect', 'payEpayChannel'] })
        ...
        <el-row>
          <el-col :xs="24" :md="20" :lg="15" :xl="12">
            <el-form-item label="支付通知地址" prop="payEpaySecret">
              <el-input v-model="formInline.payEpayNotifyUrl" placeholder="请填写支付通知地址" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :xs="24" :md="20" :lg="15" :xl="12">
            <el-form-item label="支付回调地址" prop="payEpaySecret">
              <el-input v-model="formInline.payEpayReturnUrl" placeholder="请填写支付成功后的回跳地址" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-divider />
        <el-row>
          <el-col :xs="24" :md="20" :lg="15" :xl="12">
            <el-form-item label="支付请求地址" prop="payEpayApiPayUrl">
              <el-input v-model="formInline.payEpayApiPayUrl" placeholder="请填写平台支付请求地址" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :xs="24" :md="20" :lg="15" :xl="12">
            <el-form-item label="商户查询地址" prop="payEpayApiQueryUrl">
              <el-input v-model="formInline.payEpayApiQueryUrl" placeholder="请填写平台查询商户地址" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-divider />
          ...
          <el-col :xs="24" :md="24" :lg="24" :xl="24">
            <el-form-item label="开启支付渠道" prop="payEpayChannel">
              <el-checkbox-group v-model="formInline.payEpayChannel" size="small">
                <el-checkbox v-for="item in channelList" :key="item.value" border :label="item.value">
                  {{ item.label }}
                </el-checkbox>
              </el-checkbox-group>
            </el-form-item>
