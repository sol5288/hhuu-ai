...
import { TYPEORIGINLIST, USER_STATUS_MAP, USER_STATUS_OPTIONS, USER_STATUS_TYPE_MAP } from '@/constants/index'
    ...
    <page-main>
      <el-form ref="formRef" :inline="true" :model="formInline">
        ...
        <el-form-item label="检测平台" prop="typeOriginCn">
          <el-select v-model="formInline.typeOriginCn" placeholder="请选择检测平台" clearable>
            <el-option v-for="item in TYPEORIGINLIST" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item>
      ...
      <el-table v-loading="loading" border :data="tableData" style="width: 100%;" size="large" :tooltip-options="{}">
        ...
        <el-table-column prop="status" label="用户状态" width="120" align="center">
          <template #default="{ row }: { row: { status: UserStatus } }">
            <el-tag :type="USER_STATUS_TYPE_MAP[row.userInfo.status]">
              {{ USER_STATUS_MAP[row.userInfo.status] }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="userInfo.violationCount" label="累计次数" width="90" align="center" />
