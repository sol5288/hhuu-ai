...
<template>
  <div>
    ...
    <page-main>
      <el-alert show-icon title="模型说明"
        description="所有开启的模型将会直接应用到客户端供用户选择使用、请注意在添加模型时候如果您需要使用DALL-E绘画你需要特别指定模型为绘画时候、同时例如思维导图、应用、翻译、联想等功能【目前绑定为OPENAI类目】请在添加模型的时候勾选上设置为工具Key、届时调用此类功能将使用我们指定的模型"
        type="success" />
    </page-main>
      ...
      <el-form ref="formPackageRef" v-loading="modelLoading" label-position="right" label-width="120px"
        ...
        <el-form-item :label="labelKeyName" prop="key">
          <el-input v-model="formPackage.key" :type="Number(formPackage.keyType) === 1 ? 'textarea' : 'text'" :rows="5"
            placeholder="请填写模型Key|clientId|AppId" style="width: 95%;" />
          <el-tooltip class="box-item" effect="dark" placement="right">
            <template #content>
              <div style="width: 250px;">
